...
<div id="wrapper">
  ...
  <div class="nav-user">
	    <ul>
	        ...
	        <li style="opacity:0.8;"><a href="Account.php" id= "nav-first">Account</a></li>
    ...
    <div id="right">
   <h1 style="padding:0px;" id="welcome"><?php echo $_SESSION['user_name']?>'s account</h1>   
...
if ($num = 1) { 
	...
	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
		echo '<p><div><strong>ID NUMBER:</strong> ' . $row['id'] . '<br /></div><strong>NAME:</strong> ' . $row['name'] . '<br /><strong>SURNAME: </strong>' . $row['surname'] . '<br /><strong> ID:</strong> ' . $row['sid'] . '<br /><strong>DATE OF BIRTH:</strong> ' . $row['dob'] . '<br /><strong>ADDRESS:</strong> ' . $row['address'] . '<br /><strong>CITY:</strong> ' . $row['city'] . '<br/><strong>MOBILE PHONE:</strong> ' . $row['mobile'] . '<br /><strong>YEAR: </strong>' . $row['year'] . '<br /><strong>DEPARTMENT: </strong>' . $row['dept'] . '<br /><strong>PASSWORD: </strong>' . $row['password'] . '<br /><div><strong>DATE REGISTERED: </strong>' . $row['dr'] . '</div></p>';		
	}
	
	$q1 = "SELECT b.book_title as title, DATE_FORMAT(br.date_deadline, ' %d %M %Y') AS deadline FROM users  as u, borrowings as br, books as b WHERE u.user_id = br.user_id AND br.book_id = b.book_id AND u.user_id = '$id' AND br.date_returned IS NULL";
...
if ($num1 > 0) { 
	...
	while ($row1 = mysqli_fetch_array($r1, MYSQLI_ASSOC)) {
		echo '<p><strong style="color:green;">TAKEN BOOKS:</strong> ' . $row1['title'] . '  <strong> (DEADLINE: </strong>' . $row1['deadline'] . ')</p>';		
	} 
	
	mysqli_free_result ($r1); 

} else {
	echo '<p><strong style="color:green;">You have no issued books currently.</strong><br /></p>';
}

$q2 = "SELECT r.reservation_id as id, b.book_title AS title, DATE_FORMAT(r.reservation_date, '%d %M %Y') AS date FROM reservations as r, books as b, users as u WHERE u.user_id = r.user_id AND r.book_id = b.book_id AND u.user_id = '$id'";
...
if ($num2 > 0) { 
	...
	while ($row2 = mysqli_fetch_array($r2, MYSQLI_ASSOC)) {
		echo '<p><strong style="color:green;">RESERVED BOOKS:</strong> ' . $row2['title'] . '  (<strong> ON DATE: </strong>' . $row2['date'] . ')<td align="left"><a href="DELETE_RESERVATION.php?id=' . $row2['id'] . '"><img style="margin-left:10px;" id="edit" title="Delete" src="delete_small.png"/></a></td></p>';		
	} 
	}else {
	echo '<p><strong style="color:green;">You have no reserved books currently.</strong></p>';
	}
