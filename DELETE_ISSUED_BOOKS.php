...
<head>
<meta charset="utf-8">
<title>Delete issued books</title>
...
<div id="wrapper">
  ...
  <li id="nav-rb"><a href="AllPeople.php">All people</a></li>
   ...
   <form id="addUserForm" action="DELETE_ISSUED_BOOKS.php" method="post">
      <h3 id="addNewUser"><a href="IssuedBooks.php"><img id="delete" src="delete.png" /></a> Delete borrowing</h3> 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['sure'] == 'Yes') { 
		...
		$q = "DELETE FROM borrowings WHERE borrowing_id=$id LIMIT 1";		
		$r = @mysqli_query ($dbc, $q);
		if (mysqli_affected_rows($dbc) == 1) {

			echo '<p>The borrowing has been deleted.</p>';	

		} else { 
			echo '<p class="error">The borrowing could not be deleted due to a system error.</p>'; 
			echo '<p>' . mysqli_error($dbc) . '<br />Query: ' . $q . '</p>'; 
		}
	
	} else { 
		echo '<p>The borrowing has NOT been deleted.</p>';	
	}

} else {
		
		echo "Do you want to delete this borrowing? <br />";
		
		echo '<form id="confirm" action="DELETE_USER.php" method="post" color="black">
