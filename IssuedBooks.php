...
echo '<h3 id="addNewUserAll">Issued books</h3>';
...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { 
	...
	$q = "SELECT COUNT(borrowing_id) FROM borrowings WHERE date_returned IS NULL";
...
$q = "SELECT borrowing_id, u.user_id, concat(u.user_name, ' ', u.user_surname) AS user, bk.book_title AS book, DATE_FORMAT(br.date_issued, '%d %M %Y') AS issued, DATE_FORMAT(br.date_deadline, '%d %M %y') AS deadline FROM borrowings br, users u, books bk WHERE u.user_id=br.user_id AND bk.book_id=br.book_id AND br.date_returned IS NULL ORDER BY br.date_issued ASC LIMIT $start, $display";		
$r = @mysqli_query ($dbc, $q); 
...
if ($num > 0) {
	...
	<tr><td align="left"><b>Student</b></td>
	<td align="left"><b>Book</b></td>
	<td align="left"><b>Date Issued</b></td>
	<td align="left"><b>Deadline</b></td></tr>';
	
	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
		echo '<tr><td align="left">' . $row['user'] . '</td><td align="left">' . $row['book'] . '</td><td align="left">' . $row['issued'] . '</td><td align="left">' . $row['deadline'] . '</td><td align="left"><a href="RETURN_BOOK.php?id=' . $row['borrowing_id'] . '"><img title="Return book" src="return_book_small.png"/></a></td><td align="left"><a href="DELETE_ISSUED_BOOKS.php?id=' . $row['borrowing_id'] . '"><img title="Delete" src="delete.png"/></a></td></tr>';
	}
	...
	echo '<p class="errorP">There are currently no issued books.</p>';
}
...
if ($pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="IssuedBooks.php?s=' . ($start - $display) . '&p=' . $pages . '"><img src="arrow_left.png" /></a> ';
	}
	
	for ($i = 1; $i <= $pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="IssuedBooks.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	} 
	...
	if ($current_page != $pages) {
		echo '<a href="IssuedBooks.php?s=' . ($start + $display) . '&p=' . $pages . '"><img src="arrow_right.png" /></a>';
	}
