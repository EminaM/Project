...
<div style="width:560px; padding:15px 20px;" id="showUsers">

<?php

echo '<h3 id="addNewUserAll">All book people</h3>';
...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { 
	...
	$q = "SELECT COUNT(person_id) FROM people";
...
$q = "SELECT person_id as id, person_name AS name, person_surname AS surname FROM people ORDER BY person_name asc LIMIT $start, $display";		
$r = @mysqli_query ($dbc, $q); 
...
if ($num > 0) { 
	...
	echo '<table align="center" cellspacing="3" cellpadding="3" width="600px">
	<tr><td align="left"><b>Name</b></td>
	<td align="left"><b>Surname</b></td></tr>';
	
	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
		echo '<tr><td align="left">' . $row['name'] . '</td><td align="left">' . $row['surname'] . '</td><td align="left"><a href="EDIT_PEOPLE.php?id=' . $row['id'] . '"><img id="edit" title="Edit" src="edit.png"/></a></td><td align="left"><a href="DELETE_PEOPLE.php?id=' . $row['id'] . '"><img title="Delete" src="delete.png"/></a></td></tr>';
	}
	...
	echo '<p class="errorP">There are currently no book people.</p>';
...
if ($pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="AllPeople.php?s=' . ($start - $display) . '&p=' . $pages . '"><img src="arrow_left.png" /></a> ';
	}
	
	for ($i = 1; $i <= $pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="AllPeople.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	} 
	...
	if ($current_page != $pages) {
		echo '<a href="AllPeople.php?s=' . ($start + $display) . '&p=' . $pages . '"><img src="arrow_right.png" /></a>';
	}
