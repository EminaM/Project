...
<div id="wrapper">
  ...
  <li style="background: -webkit-linear-gradient(top,#E1E9F2,white); box-shadow: 0 0 5px #ECEFF7 inset;" id="nav-rb"><a href="AllPeople.php">All people</a></li>
   ...
   <form id="addUserForm" action="DELETE_PEOPLE.php" method="post">
      <h3 id="addNewUser"><a href="AllPeople.php"><img id="delete" src="delete.png" /></a> Delete book person</h3> 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['sure'] == 'Yes') { 
		...
		$q = "DELETE FROM people WHERE person_id=$id LIMIT 1";		
		$r = @mysqli_query ($dbc, $q);
		if (mysqli_affected_rows($dbc) == 1) {

			echo '<p>The book person has been deleted.</p>';	

		} else { 
			echo '<p class="error">The book person could not be deleted due to a system error.</p>'; 
			echo '<p>' . mysqli_error($dbc) . '<br />Query: ' . $q . '</p>'; 
		}
	
	} else { 
		echo '<p>The book person has NOT been deleted.</p>';	
	}

} else {

	$q = "SELECT CONCAT(person_name, ' ', person_surname) FROM people WHERE person_id=$id";
	$r = @mysqli_query ($dbc, $q);
	...
	if (mysqli_num_rows($r) == 1) { 
		...
		echo "
		Do you want to delete <strong>$row[0]</strong> from book people? <br />";
		
		echo '<form id="confirm" action="DELETE_PEOPLE.php" method="post" color="black">
